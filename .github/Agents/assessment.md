...
- **Completion percentage**: (computed by Documentation Agent from components migrated / total components)
 - **Core details**: { "components_count": 20, "migration_target": "v19→v20", "plan_file": "plan/migration_v19_to_v20.md", "git_checkpoint_message": "chore(migration): complete Angular v20", "git_checkpoint_commit": "(populate after checkpoint - use commit hash)" }
...
- When this file or any report references `git_checkpoint_tag` (for human readability), the agent must:
  1. Create a commit with a clear, conventional message (example: `chore(migration): complete Angular v20`).
