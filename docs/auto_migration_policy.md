...
- After successful validation, create git checkpoint and push: `git status`, `git add -A`, `git commit -m "chore(migration): complete <target>"`, `git push origin main`. Do NOT create or push tags; represent the `<target>-stable` checkpoint via the commit on `main` only.
